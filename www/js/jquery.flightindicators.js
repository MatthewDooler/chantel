	...
	function FlightIndicator( placeholder, type, options ) {
		...
		placeholder.each(function(){
			switch(type){
				...
				break;
				case 'cam':
					$(this).html('<div class="instrument cam"><div class="roll box"><div class="pitch box"><div class="heading box"><img class="camimg box" alt="" /></div><img src="' + settings.img_directory + 'horizon_ball.svg" class="ball box" alt="" /></div></div><div class="mechanics box"><img src="' + settings.img_directory + 'horizon_mechanics_simple.svg" class="box" alt="" /></div></div>');
					_setHeading(settings.heading);
			...
			//$(this).find('div.instrument.cam').css({height : settings.size, width : settings.size*4});
		...
		function _setCamRoll(roll){
			placeholder.each(function(){
				$(this).find('div.instrument.cam div.roll').css('transform', 'rotate('+roll+'deg)');
			});
		}

		function _setCamPitch(pitch){
			if(pitch>constants.cam_pitch_bound){pitch = constants.cam_pitch_bound;}
			else if(pitch<-constants.cam_pitch_bound){pitch = -constants.cam_pitch_bound;}
			placeholder.each(function(){
				$(this).find('div.instrument.cam div.roll div.pitch').css('top', pitch*0.7 + '%');
			});
		}

		function _setCamHeading(heading){
			placeholder.each(function(){
				relativeHeading = heading - self.camImageHeading
				$(this).find('div.instrument.cam div.roll div.pitch div.heading').css('left', relativeHeading*3.0 + '%');
			});	
		}

		function _setCamImage(roll, pitch, heading, uri){
			self.camImageRoll = roll;
			self.camImagePitch = pitch;
			self.camImageHeading  = heading;
			placeholder.each(function(){
				var container = $(this).find('div.instrument.cam div.roll div.pitch')
				var camimg = container.find('.camimg')
				camimg.attr("src", uri);
				var camimg_width = camimg.width();
				var camimg_height = camimg.height();

				// TODO: pitch*3 is almost definitely wrong - needs to be a % and needs testing
				var topMargin = (-(camimg.height()/2))+(container.height()/2)-(pitch*3)
				camimg.css('top', topMargin+"px");

				var leftMargin = (-(camimg.width()/2))+(container.width()/2)
				camimg.css('left', leftMargin+"px");

				// compensate for roll too (pre-rotate)
				// TODO: might be wrong - needs testing
				camimg.css('transform', 'rotate('+roll+'deg)');
			});
		}
		...
		this.setCamRoll = function(roll){_setCamRoll(roll);}
		this.setCamPitch = function(pitch){_setCamPitch(pitch);}
		this.setCamHeading = function(heading){_setCamHeading(heading);}
		this.setCamImage = function(roll, pitch, heading, uri){_setCamImage(roll, pitch, heading, uri);}
